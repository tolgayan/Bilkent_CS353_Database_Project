...
db.query("CREATE VIEW IF NOT EXISTS completed_reports AS \
         SELECT image, footballer.forename, final_report.rating, final_report.comment, scout.scout_id, report_id, \
         ...
         final_report.scout_id=assignment.scout_id AND final_report.task_id=assignment.task_id AND scout.scout_id=assignment.scout_id) AND \
...
router.get('/', function(req, res, next) {
    db.query("SELECT club_name, task_id, height, weight, age, position, time, scout_num, foot,\
                ...
                scout_id=2 AND club.id=task.club_id AND task.status='uncomplete' ORDER BY assigned_date", (err, result) => {
        if (err) {
            throw err;
            console.log("error");
        }
...
function aFunction(req, res){
    if(req.body.status == "incomplete" ){
        console.log(" 4q3t4w");
        db.query("SELECT * FROM completed_reports WHERE scout_id='1'",(err, result) => {
            if (err) {
                throw err;
                console.log("error");
            }
...
router.get('/posts', function(req, res, next) {
    db.query("SELECT * FROM completed_reports WHERE scout_id='2'",(err, result) => {
        if (err) {
            throw err;
            console.log("error");
        }
